...
- (id)initWithFrame:(CGRect)frame
{
    ...
    {
        self.frame = CGRectMake(frame.origin.x, frame.origin.y, 117, 117);
        [self commonInit];
    }
...
- (void)rockerValueChanged
{
    ...
    if (-1 != rockerDirection && rockerDirection != _direction) {
        ...
        if ([self.delegate respondsToSelector:@selector(axcRockerDidChangeDirection:)])
        {
            [self.delegate axcRockerDidChangeDirection:self];
        }
    }
}
@end
