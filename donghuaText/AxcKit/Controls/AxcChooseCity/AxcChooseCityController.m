...
#import "AxcCityGroupCell.h"
#import "AxcCityHeaderView.h"
#import <CoreLocation/CoreLocation.h>
#import <MapKit/MapKit.h>

@interface AxcChooseCityController ()<AxcCityGroupCellDelegate,UISearchBarDelegate,CLLocationManagerDelegate>
/**
 *  记录所有城市信息，用于搜索
 */
...
@implementation AxcChooseCityController
...
//    self.searchBar.barStyle     = UIBarStyleDefault;
    ...
    [self.tableView registerClass:[AxcCityGroupCell class] forCellReuseIdentifier:cityGroupCell];
    [self.tableView registerClass:[AxcCityHeaderView class] forHeaderFooterViewReuseIdentifier:cityHeaderView];
}
...
- (NSMutableArray *) localCityData
{
    if (_localCityData == nil) {
        ...
        if (self.locationCityID != nil) {
            AxcCity *city = nil;
            for (AxcCity *item in self.recordCityData) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    AxcCityGroup *group = [self.cityDatas objectAtIndex:section - 3];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isSearch) {
        ...
        AxcCity *city =  [self.searchCities objectAtIndex:indexPath.row];
        [cell.textLabel setText:city.cityName];
        return cell;
    }
    
    if (indexPath.section < 3) {
        AxcCityGroupCell *cell = [tableView dequeueReusableCellWithIdentifier:cityGroupCell];
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cityCell];
    AxcCityGroup *group = [self.cityDatas objectAtIndex:indexPath.section - 3];
    AxcCity *city =  [group.arrayCitys objectAtIndex:indexPath.row];
    [cell.textLabel setText:city.cityName];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    AxcCityHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:cityHeaderView];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        return [AxcCityGroupCell getCellHeightOfCityArray:self.localCityData];
    }
    else if (indexPath.section == 1) {
        return [AxcCityGroupCell getCellHeightOfCityArray:self.commonCityData];
    }
    else if (indexPath.section == 2){
        return [AxcCityGroupCell getCellHeightOfCityArray:self.hotCityData];
    }
    return 44.0f;
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if (searchText.length == 0) {
        ...
        self.isSearch = YES;
        for (AxcCity *city in self.recordCityData){
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(axcCityPickerControllerDidCancel:)]) {
        [_delegate axcCityPickerControllerDidCancel:self];
    }
}
#pragma mark - Private Methods
- (void) didSelctedCity:(AxcCity *)city
{
    if (_delegate && [_delegate respondsToSelector:@selector(axcCityPickerController:didSelectCity:)]) {
        [_delegate axcCityPickerController:self didSelectCity:city];
    }
