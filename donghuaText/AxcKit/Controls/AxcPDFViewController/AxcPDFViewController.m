...
@interface AxcPDFViewController ()<UIAlertViewDelegate>{
    BOOL next_;     // 是否翻向下一页
...
@implementation AxcPDFViewController
...
- (void)viewDidLoad {
    ...
    /* 初始化视图 */
    self.navigationItem.title = [NSString stringWithFormat:@"%ld / %ld", currentPage_, totalPages_];
    self.curView  = [[AxcPDFView alloc] initWithPDFRef:pdfRef_ andLastPage:self.lastPage];
    self.addView  = [[AxcPDFView alloc] initWithPDFRef:pdfRef_ andLastPage:self.lastPage];
    self.backView = [[AxcPDFView alloc] initWithPDFRef:pdfRef_ andLastPage:self.lastPage];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (currentPage_ == totalPages_) {
        if (next_ == YES) {
            if ([self.Delegate respondsToSelector:@selector(axcReadThePDFToTheEnd)]) {
                [self.Delegate axcReadThePDFToTheEnd];
            }
            UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"注意" message:@"已经到达最后一页" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:@"返回", nil];
            [av show];
            av.delegate = self;
            av.tag = 5324;
            return;
    ...
    if (next_ == YES) { // 下一页
        currentPage_++;
        NSLog(@"%ld / %ld", currentPage_, totalPages_);
        self.curView.frame = fitRect_;
        self.curView.pageIndex = currentPage_;
        [self.curView reloadView];
...
-(void)backToLast{
    if([self.Delegate respondsToSelector:@selector(axcClosePDFViewWithCurrentPage:)]){
        [self.Delegate axcClosePDFViewWithCurrentPage:currentPage_];
        [self.navigationController popViewControllerAnimated:YES];
    }
}

- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (alertView.tag == 5324) {
        if (buttonIndex == 1) {
            [self.navigationController popViewControllerAnimated:YES];
        }
    }
