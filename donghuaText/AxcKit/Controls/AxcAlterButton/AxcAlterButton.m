...
@interface AxcAlterButton()<AxcAlterItemButtonDelegate>
...
- (void)addButton:(AxcAlterItemButton *)button {
...
- (void)addButtonItems:(NSArray *)itemButton
{
    ...
    for (AxcAlterItemButton *item in itemButton) {
        [self addButton:item];
    }
}
...
- (void)centerButtonOpen
{
    ...
    for (int i = 0;  i < self.items.count; i++) {
        ...
        AxcAlterItemButton *itemButton = self.items[i];
        itemButton.index = i;
        
        [CATransaction setCompletionBlock:^{
            
            for (AxcAlterItemButton *itemButton in self.items) {
...
- (void)centerButtonClosed
{
    ...
    [CATransaction setCompletionBlock:^{
        ...
        for (AxcAlterItemButton *button in self.items) {
    ...
    for (int i = (int)self.items.count - 1; i>=0; i--) {
        ...
        AxcAlterItemButton *button = self.items[i];
...
- (void)itemButtonClicked:(AxcAlterItemButton *)itemButton
{
    if ([self.delegate respondsToSelector:@selector(AlterButton:clickItemButtonAtIndex:)]) {
        ...
        AxcAlterItemButton *selectedButton = self.items[itemButton.index];
