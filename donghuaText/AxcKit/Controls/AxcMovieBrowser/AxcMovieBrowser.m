...
@interface AxcMovieBrowser () <UIScrollViewDelegate>
...
@implementation AxcMovieBrowser
...
- (void)commonInit
{
    ...
    if (self.movies.count > 0) {
        [_backgroundView sd_setImageWithURL:[NSURL URLWithString:((AxcMovie *)self.movies[0]).coverUrl]];
    }
...
- (void)setupScrollView
{
    ...
    for (AxcMovie *movie in self.movies) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.currentIndex < self.movies.count) {
        [self.backgroundView sd_setImageWithURL:[NSURL URLWithString:((AxcMovie *)self.movies[self.currentIndex]).coverUrl]];
