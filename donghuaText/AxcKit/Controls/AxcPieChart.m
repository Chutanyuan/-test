...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.selectedLayer == [self getLayerWithIndex:index]) {
        ...
        if ([_delegate respondsToSelector:@selector(AxcPieChart:index:finished:)]) {
            [_delegate AxcPieChart:self index:index finished:NO];
        }
        return;
    }
    [UIView animateWithDuration:0.2 animations:^{
    ...
    if ([_delegate respondsToSelector:@selector(AxcPieChart:index:finished:)]) {
        [_delegate AxcPieChart:self index:index finished:YES];
    }
}
