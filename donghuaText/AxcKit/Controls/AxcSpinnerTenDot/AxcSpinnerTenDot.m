...
// Blur View
@property (strong, nonatomic) AxcBlurView *backgroundBlur;
...
@end
@implementation AxcSpinnerTenDot
...
-(void) initBackgroundWithBlur:(BOOL)blur
{
    ...
    if (_isShouldBlur)
    {
        _backgroundBlur = [[AxcBlurView alloc] initWithFrame:_containerView.bounds];
...
-(void) initDot
{
    ...
    {
        AxcTenDot *dot = [[AxcTenDot alloc] initDotAtMainView:self atIndex:i];
...
-(void) show
{
    ...
    // Call Delegate
    if ([_delegate respondsToSelector:@selector(AxcSpinnerTenDotWillShow:)])
    {
        [_delegate AxcSpinnerTenDotWillShow:self];
    }
                     ...
                     } completion:^(BOOL finished)
    {
        _isAnimating = YES;
        // Call Delegate
        if ([_delegate respondsToSelector:@selector(AxcSpinnerTenDotDidShow:)])
        {
            [_delegate AxcSpinnerTenDotDidShow:self];
        }
...
-(void) dismiss
{
    ...
    [UIView animateWithDuration:0.4f delay:0
                        options:UIViewAnimationOptionCurveEaseIn animations:^{
                            ...
                            // Call delegate
                            if ([_delegate respondsToSelector:@selector(AxcSpinnerTenDotDidDismiss:)])
                            {
                                [_delegate AxcSpinnerTenDotDidDismiss:self];
                            }
